...
type PipelineEvent =
  ...
  | {
      type: "run_status";
      data: { run_id: string; status: string; [key: string]: any };
    }
...
const messageQueue: PipelineEvent[] = [];
    ...
    websocketStore.update((state) => {
      ...
      try {
        const wsUrl =
          ...
          `/ws/logs?token=${encodeURIComponent(token)}`;
            ...
            if (newAttempts < s.maxReconnectAttempts) {
              ...
              onLog?.(
                ...
                `Attempting to reconnect in ${
                  delay / 1000
                }s... (${newAttempts}/${s.maxReconnectAttempts})`
              );
